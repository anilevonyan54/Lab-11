...
DWORD WINAPI ClientHandler(LPVOID arg) {
    ...
    while (true) {
        ...
        int bytes_received = recv(clientInfo->newfd, readbuf, sizeof(readbuf), 0);
        if (bytes_received <= 0) {
            if (bytes_received == 0) {
        ...
        std::string receivedMessage = std::string(clientInfo->name) + ": " + std::string(readbuf);
...
int main() {
    ...
    for (int i = 0; i < MAX_CLIENT; i++) {
        ...
        CreateThread(NULL, 0, ClientHandler, (LPVOID)&Clients[i], 0, NULL);
    }
